...
| Constant | Value | Description |
|----------|-------|-------------|
| `AM_DEPTH_MIN` | 0 | Minimum AM depth % |
| `AM_DEPTH_MAX` | 120 | Maximum AM depth % |
| `DUTY_CYCLE_MIN` | 20 | Minimum duty cycle % |
| `DUTY_CYCLE_MAX` | 80 | Maximum duty cycle % |
| `BURST_PHASE_MIN` | -360 | Minimum burst phase degrees |
| `BURST_PHASE_MAX` | 360 | Maximum burst phase degrees |
...
```python
awg.set_byte_order(HP33120A.BYTE_ORDER_NORMAL)  # MSB first
awg.set_byte_order(HP33120A.BYTE_ORDER_SWAPPED)  # LSB first
order = awg.get_byte_order()  # Returns "NORM" or "SWAP"
```

Note: `upload_waveform_dac()` automatically sets byte order to NORMAL before upload.
...
# Additional waveform attributes
avg = awg.get_waveform_average()           # Average value
avg = awg.get_waveform_average("NAME")     # Named waveform
crest = awg.get_waveform_crest_factor()    # Crest factor (peak/RMS)
ptp = awg.get_waveform_ptp()               # Peak-to-peak value
```
...
Note: Parameters are validated - `set_am_depth()` accepts 0-120%, `set_duty_cycle()` accepts 20-80%, `set_burst_phase()` accepts -360 to +360 degrees.

```python
awg.set_am_depth(50)  # 0-120% (validated)
